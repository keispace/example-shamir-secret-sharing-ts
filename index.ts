...
export const stringToShards64 = async (
    ...
    // const secret = new TextEncoder().encode(compressed);
    const shards = await split(compressed, shares, threshold);
    return shards.map((shard) => baseCodec.encode(shard));
};

export const shards64ToString = async (shardBases: string[], base: SupportedBase = 'base64'): Promise<string> => {
...
export const test = async () => {
    ...
    const shards = await stringToShards64(originString, shares, threshold, base);
